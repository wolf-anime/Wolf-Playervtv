...
class WolfPlayer {
  ...
  constructor(selector, options = {}) {
    ...
    this.options = {
      ...
      title: options.title || 'Sin título',
...
`;
  ...
  async loadSource(url) {
    if (!url) return;
    this.destroyAll();
    this.videoTitle.textContent = decodeURIComponent(url.split('/').pop().split('?')[0]) || url;
  ...
  updateQualityActive(level) {
    ...
    if (this.selectQuality.value === '-1') {
      const opt = this.selectQuality.querySelector(`option[value="${level}"]`);
      if (opt) {
        this.labelQuality.textContent = `Auto (${opt.textContent})`;
      }
    }
  }
